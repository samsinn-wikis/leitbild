...
For process-plant procedures, resolve procedure tags through the process-plant signal query surface. Use `process-plant.signals.read` with an explicit `systemId` and one or more `{ "tagId": "..." }` references. If a procedure document includes a tag appendix, use `process-plant.procedure-tags.validate` first to catch missing tags or metadata mismatches before relying on the document. Do not assume a current unit, do not infer tags from labels, and do not use fleet-wide shortcuts. Check the returned signal capabilities before treating a value as procedure-relevant, trendable, alarmable, or writable. Before changing a writable process value, prefer `process-plant.control.validate` to dry-run signal resolution, type/range validation, and I&C permissive/interlock gates. Then use `process-plant.control.write` with exactly one reference form: either `path` or `tagId`; writes outside declared hard ranges or blocked by I&C permissives/interlocks are rejected.
