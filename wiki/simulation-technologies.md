...
### Licensing Reality And The Medium-Fidelity Path

Licensing matters here because Leitbild should remain shareable, inspectable, and easy to deploy. RELAP5-3D, TRACE, NRC RELAP5, PARCS, SNAP, and related reactor safety-code ecosystems should be treated as restricted or controlled references rather than dependencies. They may be available to qualified organizations through license agreements, CAMP/RAMP-style arrangements, export-control review, nondisclosure terms, or other access restrictions, but that is exactly the wrong shape for an open Leitbild pack. We should not build a future Leitbild NPP pack around a solver that colleagues, students, AI agents, or public wiki readers cannot freely inspect and run.

The open-source landscape is better, but still not a silver bullet. MOOSE is open source and technically serious, but it is a large C++ multiphysics framework, not a lightweight process-control simulator. OpenModelica and the Modelica Standard Library are attractive because Modelica is built around component networks, equations, fluids, thermal systems, and controls; the Modelica Standard Library is permissive, while OpenModelica itself needs careful license review depending on how it is embedded or distributed. OpenFOAM is open source and powerful for CFD, but it is too low-level and heavy for a first process-control pack. FMI is not an engine; it is a boundary standard. It becomes useful when we want to run an FMU produced by a Modelica or other modeling tool and connect it to Leitbild as a co-simulation provider.

The practical conclusion is that Leitbild should not depend on RELAP or TRACE, and it should not start by embedding MOOSE or OpenFOAM. The first NPP/process pack should be a medium-fidelity operational model written in TypeScript. Its purpose would be to produce plausible, consistent process-control behavior for research, training, interaction design, AI-agent testing, and scenario coordination. It would not claim licensing-grade thermohydraulic fidelity. This is an important distinction: the model should be good enough to support operator-facing displays, alarms, procedure branches, system interactions, and cross-pack consequences, but honest about its simplifications.

Such a model is realistic if we choose the right abstraction. We should model plant systems as connected components with state, limits, control logic, alarms, and simplified flow/energy relationships. A pump can add head or fail. A valve can change path availability. A tank or steam generator can have level, pressure, inflow, and outflow. A pressurizer can have pressure bands, heater/spray effects, and relief behavior. A diesel generator can have start delay, load state, fuel dependence, failure state, and bus connection. A reactor trip can reduce power while decay heat remains. These are not high-fidelity conservation-equation solvers, but they can create functional process behavior that is meaningful to an operator.

The initial component set should cover the systems that drive rich scenarios: offsite power, safety buses, diesel generators, batteries, reactor trip state, simplified decay heat, reactor coolant pumps, primary temperature/pressure, pressurizer level and pressure, steam generator level and pressure, main feedwater, auxiliary feedwater, safety injection, residual heat removal, containment pressure, radiation indicators, alarms, procedure state, and operator action history. The model should expose stable signal names such as `npp.offsitePower.available`, `npp.dg.a.status`, `npp.sg.a.level.percent`, `npp.pressurizer.pressure.bar`, `npp.safetyInjection.active`, and `npp.procedure.active`. If a later FMU or MOOSE-based engine replaces the internals, those signal names and command contracts should remain valid.

The implementation should be deterministic and testable. A plant pack should advance on the Leitbild run clock, use explicit time steps, avoid hidden wall-clock behavior, and record meaningful events rather than every small numerical change. Tests should check invariants such as "a pump cannot produce flow when unavailable," "offsite power loss transfers buses only through declared logic," "steam generator level responds directionally to feedwater and heat removal," and "procedure cues follow explicit alarm/signal conditions." This is also where AI-assisted or vibe-coded development can be useful: an LLM can draft one subsystem at a time from public source material and the PWR Ops wiki, but every subsystem must include assumptions, limitations, signal definitions, and tests before it becomes part of the pack.

The future upgrade path should be adapter-based. If we later need more formal thermohydraulics, the pack can wrap an OpenModelica/FMU model, a MOOSE application, or another open engine behind the same provider contract. The pack's public boundary would remain the same: declared commands, signals, events, snapshots, health status, and surfaces. This protects Leitbild from solver churn and protects the simulation from being flattened into generic map objects.

Leitbild's path should therefore be two-layered. For operational research scenarios, we can build simplified TypeScript process models that represent plant behavior at a coarse, transparent level: offsite power, diesel generators, steam generator levels, pressurizer pressure bands, safety injection state, procedure state, alarms, and external support requests. These models should carry explicit limitations and source links to the [PWR Ops wiki](https://samsinn-wikis.github.io/pwr-ops/). For higher-fidelity dynamics, the pack should wrap open Modelica/FMU, MOOSE, or other open/suitably licensed engines behind the same published signal contract. Restricted reactor safety codes should remain references, not Leitbild dependencies.
